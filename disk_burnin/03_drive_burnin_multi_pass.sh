...
# PHASE 04: WRITE TEST (badblocks) - Rigorous 4-Pass Version
if should_run PHASE_04_WRITE_TEST; then
    echo "[$(date)] Phase 04: Parallel 4-Pass badblocks Write Test..."
    pids=()
    for d in "${DRIVES[@]}"; do
        # Removing '-t 0x00' tells badblocks to run the full 4-pattern suite
        badblocks -w -s -b 4096 "$d" > "$LOG_DIR/badblocks_$(basename "$d").log" 2>&1 &
        pids+=($!)
    done
